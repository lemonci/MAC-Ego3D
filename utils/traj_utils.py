...
class TrajManager:
    def __init__(self, which_dataset, dataset_path):
        ...
        # print("dtatpath:",self.dataset_path)
        if self.which_dataset == "tum":
            self.gt_poses = self.tum_load_poses(self.dataset_path + '/traj.txt')
        elif self.which_dataset == "replica" or self.which_dataset == "real":
    ...
    def save_traj(self, est, path):
        ...
        poses : list of estimated poses
        '''
        traj = np.array([x[:3, 3] for x in self.gt_poses])
        # self.gt_poses_align[:, :3, :3] = np.einsum('ij,njk->nik', rot_align, self.gt_poses[:, :3, :3])
        pyplot.clf()
        pyplot.title(f'Seq {iter}')
        pyplot.plot(est[:, 0], est[:, 1], label='estimated trajectory', linewidth=5)
        pyplot.legend()
        pyplot.plot(traj[:,0], traj[:, 1], label='g.t. trajectory', linewidth=2)
        pyplot.legend()
        pyplot.axis('equal')
        pyplot.savefig(path)
