...
class Tracker(SLAMParameters):
    ...
    def input_transform(self, img_tensor, image_size=None):
        ...
        img_tensor = (img_tensor.to('cuda') - MEAN) / STD
        return img_tensor
    ...
    def rawPose2GTSAMPose(self, rawPose):
        x, y, z = rawPose[0:3, 3]
        R = rawPose[0:3, 0:3]
        # Clamp the trace value to avoid numerical errors
        trace_val = 1 + R[0, 0] + R[1, 1] + R[2, 2]
        trace_val = np.clip(trace_val, 0, 4)  # Valid range for rotation matrix trace
        qw = 0.5 * np.sqrt(trace_val)
        if qw < 1e-6:  # Handle near-zero quaternion w
            qw = 1e-6
        qx = (R[2, 1] - R[1, 2]) / (4 * qw)
        qy = (R[0, 2] - R[2, 0]) / (4 * qw)
        qz = (R[1, 0] - R[0, 1]) / (4 * qw)
    ...
    def rotation_matrix_difference(self, R1, R2):
        ...
        # Clamp the value to the valid range for arccos [-1, 1]
        cos_angle = np.clip((trace_value - 1) / 2, -1.0, 1.0)
    ...
    def tracking(self):
        ...
        for agentIndex in range(len(self.final_pose)):
            ...
            # Convert list of numpy arrays to a single numpy array first, then to tensor
            self.final_pose[agentIndex][:,:,:] = torch.from_numpy(np.array(self.poses[agentIndex])).float()
