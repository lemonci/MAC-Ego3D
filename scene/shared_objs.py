...
class SharedGaussians(nn.Module):
    def __init__(self, num_points):
        ...
        self.frame_idx = torch.zeros((1)).int().cuda()

    def input_values(self, new_xyz, new_colors, new_rots, new_scales, new_z_values, new_trackable_filter, frame_idx):
        # on CPU memory
...
class SharedCam(nn.Module):
    def __init__(self, FoVx, FoVy, image, depth_image,
        ...
        self.agent_idx = torch.zeros((1)).int()
