...
class GaussianModel(nn.Module):
    ...
    def __init__(self, sh_degree : int):
        ...
        self.point_frame_idx = torch.empty(0)
    ...
    def restore(self, model_args, training_args):
        ...
        opt_dict, 
        self.spatial_lr_scale,
        self.point_frame_idx) = model_args
    ...
    @property
    def get_point_frame_idx(self):
        return self.point_frame_idx
    ...
    def create_from_pcd2_tensor(self, points, colors, rots_, scales_, z_vals_, trackable_idxs, frame_idx):
        ...
        self.point_frame_idx = torch.full((self.get_xyz.shape[0],), frame_idx.item(), dtype=torch.int, device="cuda")        
        self.keyframe_idx = torch.ones((self.get_xyz.shape[0],1), dtype=torch.bool, device="cuda")
        
        torch.cuda.empty_cache()
    
    def add_from_pcd2_tensor(self, points, colors, rots_, scales_, z_vals_, trackable_idxs, frame_idx):
        ...
        self.new_point_frame_idx = torch.full((self.new_xyz.shape[0],), frame_idx.item(), dtype=torch.int, device="cuda")
        ...
        self.densification_postfix(self.new_xyz, self.new_features_dc, 
                                   ...
                                   self.new_scaling, self.new_rotation, self.new_trackable_mask, self.new_point_frame_idx)
    ...
    def prune_points(self, mask):
        ...
        self.point_frame_idx = self.point_frame_idx[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_trackable_mask, new_point_frame_idx):
        d = {"xyz": new_xyz,
        ...
        self.point_frame_idx = torch.concat([self.point_frame_idx,new_point_frame_idx], dim=0)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # new_trackable_mask[:] = 0
        new_point_frame_idx = self.point_frame_idx[selected_pts_mask].repeat(N)
        new_point_frame_idx[:] = -1
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_trackable_mask, new_point_frame_idx)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        # new_trackable_mask[:] = 0
        new_point_frame_idx = self.point_frame_idx[selected_pts_mask]
        new_point_frame_idx[:] = -1
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_trackable_mask, new_point_frame_idx)
        # torch.cuda.empty_cache()
    ...
    def prune_large_and_transparent(self, min_opacity, extent):
        ...
        if extent != None:
            big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * extent
            big_points_ws_2 = self.get_scaling.max(dim=1).values > (torch.sum(self.get_scaling,dim=1) - self.get_scaling.max(dim=1).values) * 10
            prune_mask = torch.logical_or(prune_mask, big_points_ws)
            prune_mask = torch.logical_or(prune_mask, big_points_ws_2)
        self.prune_points(prune_mask)
        # print(self.get_scaling.shape, self.get_scaling.max(dim=1).values.max(dim=0))
