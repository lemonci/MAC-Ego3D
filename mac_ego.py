...
class MAC_EGO(SLAMParameters):
    def __init__(self, args):
        super().__init__()
        self.dataset_path = args.dataset_path.split(';')
        # print(self.dataset_path, len(self.dataset_path))
        ...
        self.ifma = int(args.mu)
        self.ifnoise = float(args.noise)
        self.lc_freq = float(args.lc_freq)
        self.post_opt = float(args.post_opt)
        self.test = args.test
        ...
        num_final_poses = [] #store the pose-len of each video
        for item in self.dataset_path:
            # print(item)
            self.trajmanager.append(TrajManager(self.camera_parameters[8],item))
            num_final_poses.append(len(self.trajmanager[-1].gt_poses))
        
        
        # Make test cam
        # To get memory sizes of shared_cam
        test_rgb_img, test_depth_img = self.get_test_image(f"{self.dataset_path[0]}/images")
        #print(test_rgb_img.shape,test_depth_img.shape)        
        ...
        self.final_pose = []
        for x in num_final_poses:
            self.final_pose.append(torch.zeros((x,4,4)).float())
            self.final_pose[-1].share_memory_()
    ...
    def get_test_image(self, images_folder):
        ...
        if self.camera_parameters[8] == "replica" or self.camera_parameters[8] == "real":
            images_folder = os.path.join(self.dataset_path[0], "images")
            ...
            rgb_image = cv2.imread(f"{self.dataset_path[0]}/images/{image_name}.jpg")
            depth_image = np.array(o3d.io.read_image(f"{self.dataset_path[0]}/depth_images/{depth_image_name}.png")).astype(np.float32)
            rgb_image = cv2.resize(rgb_image, (depth_image.shape[1],depth_image.shape[0]))

        elif self.camera_parameters[8] == "tum":
            rgb_folder = os.path.join(self.dataset_path[0], "rgb")
            depth_folder = os.path.join(self.dataset_path[0], "depth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--overlapped_th2", default=5e-4)
    ...
    parser.add_argument('--cuda', type=str, default="0", help="Comma-separated list of CUDA device numbers to use, e.g., '0', '1', '0,1'")
    parser.add_argument('--mu', default=0, help="Whether use multiple agent 0 for not, 1 for yes")
